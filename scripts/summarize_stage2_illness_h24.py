...
RAW_RESULTS = Path("results/stage2_illness_h24/raw_results.csv")
SUMMARY_CSV = Path("results/stage2_illness_h24/summary_mean_std_stderr.csv")
SUMMARY_MD = Path("results/stage2_illness_h24/summary_mean_std_stderr.md")
...
def wide_results(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if metric_col and value_col:
        ...
        wide = df.rename(columns={"adapter_normalized": "adapter_normalized"}).copy()
...
def metric_column(df: pd.DataFrame, candidates: list[str]) -> str:
    ...
    if column is None:
        raise ValueError(
            f"Could not identify metric column from candidates {candidates}. "
...
def markdown_table(summary: pd.DataFrame) -> str:
    ...
    for column in display.columns:
        if column != "adapter" and column != "n":
